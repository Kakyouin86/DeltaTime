...
inline void CalculateFinalFog(float noise, float2 uv, inout float fogFactor, inout fixed3 fogColor)
{
	...
	ApplyDither(fogColor.rgb, uv, fixed3(0.06711056, 0.00583715, 52.9829189), _WeatherMakerDitherLevel * (1.0 / fogFactor));
}
...
inline void RaycastFogBoxFullScreen(float3 rayDir, float3 forwardLine, out float3 worldPos, inout float depth, out float noise)
{
	...
	float fogYNoise = CalculateNoiseXZ(_FogNoiseHeight, worldPos, _FogNoiseHeightScale, _FogNoiseVelocity, _FogNoiseHeightMultiplier, 0.5) * inBoxMultiplier * _FogNoiseHeightVariance;
	...
	noise = CalculateNoiseXZ(_FogNoise, worldPos, _FogNoiseScale, _FogNoiseVelocity, _FogNoiseMultiplier, 0.5) * inBoxMultiplier;

	// and finally calculate noise looking in the box out
	float3 rayExitPoint = _WorldSpaceCameraPos + (rayDir * min(depth, 100));
	noise += CalculateNoiseXZ(_FogNoise, rayExitPoint, _FogNoiseScale, _FogNoiseVelocity, _FogNoiseMultiplier, 0.5) * 0.1 * (1.0 - inBoxMultiplier);
	...
	float3 rayExitPoint = _WorldSpaceCameraPos + (rayDir * min(depth, 100));
	noise = CalculateNoiseXZ(_FogNoise, rayExitPoint, _FogNoiseScale, _FogNoiseVelocity, _FogNoiseMultiplier * 0.1, 0.5);
...
fog_full_screen_fragment fog_full_screen_vertex_shader(fog_vertex v)
{
	...
	_WeatherMakerSunDirection.xy = UnityStereoScreenSpaceUVAdjust(_WeatherMakerSunDirection.xy, _MainTex_ST);
...
fixed4 fog_box_full_screen_fragment_shader(fog_full_screen_fragment i) : SV_TARGET
{
	...
	float depth = GetDepth01(i.uv.xy);
	float noise;
	float3 worldPos;
	...
	float sunAmount = saturate(dot(rayDir, _WeatherMakerSunDirection));
	float sunIntensity;

#if defined(ENABLE_FOG_HEIGHT) || defined(ENABLE_FOG_HEIGHT_WITH_NOISE)

	sunIntensity = _WeatherMakerSunColor.a;

#else

	// dim sun as fog gets more dense
	sunIntensity = _WeatherMakerSunColor.a * saturate(((1.0 - _FogDensity) + 0.35));
	...
	float3 fogColor = lerp(_FogColor.rgb * _WeatherMakerSunColor.rgb * sunIntensity * noise, _WeatherMakerSunColor.rgb * sunIntensity, sunLerp);

#else

	float3 fogColor = _FogColor.rgb * _WeatherMakerSunColor.rgb * _WeatherMakerSunColor.a * noise;
