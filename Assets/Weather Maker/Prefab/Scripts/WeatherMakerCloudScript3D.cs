    ...
    public class WeatherMakerCloudScript3D : WeatherMakerCloudScript
    {
        private class CloudGenerator
        {
            ...
            public void CreateCloud()
            {
                ...
                maxLifeTime = Mathf.Max(maxLifeTime, lifeTime.x + fadeTime);
            ...
            public float CreateClouds()
            {
                ...
                return maxLifeTime - Time.timeSinceLevelLoad;
            }
            ...
            /// <returns>The total time to show the clouds</returns>
            public static float ShowClouds(Mesh mesh, RangeOfFloats lifeTimeRange, RangeOfFloats fadeTimeRange, RangeOfFloats fadeTimeDelayRange)
            {
                if (mesh == null)
                {
                    return 0.0f;
                ...
                for (int i = 0; i < mesh.vertexCount / 4; i++)
                {
                    ...
                    maxLifeTime = Mathf.Max(maxLifeTime, fadeDelay + fadeTime);
            ...
            /// <param name="fadeTimeRange">Range of fade time for each cloud</param>
            /// <param name="fadeTimeDelayRange">Amount to add to the delay for fading for each subsequent cloud</param>
            /// <returns>The total time before the clouds will be faded out</returns>
            public static float HideClouds(Mesh mesh, RangeOfFloats fadeTimeRange, RangeOfFloats fadeTimeDelayRange)
            {
                if (mesh == null)
                {
                    return 0.0f;
            ...
            public bool ShowingClouds { get; private set; }
        }
        ...
        protected override void Start()
        {
            ...
            mesh = new Mesh()
            {
                name = "WeatherMakerClouds"
            };
            meshFilter.sharedMesh = mesh;
            WeatherScript.DayNightScript.SunIntensityMultipliers["WeatherMakerCloudScript3D"] = 1.0f;
            base.Start();
        }
        ...
        /// <summary>
        /// Start the clouds appearing - call Reset first to create new types of clouds
        /// </summary>
        public override void CreateClouds()
        {
            ...
            if (mesh.vertexCount == 0)
            {
                CloudGenerator cloudGenerator = new CloudGenerator(mesh, NumberOfClouds, MaterialRows, MaterialColumns)
                {
                    ...
                    BoundsHeightRange = BoundsHeightRange,
                    FadeTimeRange = FadeTimeRange,
                    FadeTimeDelayRange = FadeTimeDelayRange,
                    LifeTimeRange = LifeTimeRange,
                    RadiusRange = RadiusRange,
                    VelocityRangeX = VelocityRangeX,
                    VelocityRangeZ = VelocityRangeZ
                };
                timeToCreate = cloudGenerator.CreateClouds();                
                cloudGenerator.Apply();
            }
            else
            {
                timeToCreate = CloudGenerator.ShowClouds(mesh, LifeTimeRange, FadeTimeRange, FadeTimeDelayRange);
            }
            WeatherScript.SkySphereScript.SetFlareEnabled(false);
            TweenFactory.Tween("WeatherMakerSunIntensity", WeatherScript.DayNightScript.SunIntensityMultipliers["WeatherMakerCloudScript3D"], 0.5f, timeToCreate, TweenScaleFunctions.Linear, (ITween<float> c) =>
            {
                WeatherScript.DayNightScript.SunIntensityMultipliers["WeatherMakerCloudScript3D"] = c.CurrentValue;
                WeatherScript.DayNightScript.SunShadowIntensityMultipliers["WeatherMakerCloudScript3D"] = Mathf.Lerp(1.0f, 0.0f, (1.0f - c.CurrentValue) * 2.0f);
            }, null);
        }
        ...
        public override void RemoveClouds()
        {
            ...
            {
                ShowingClouds = false;
                float totalTimeToRemove = CloudGenerator.HideClouds(mesh, FadeTimeRange, FadeTimeDelayRange);
                WeatherScript.SkySphereScript.SetFlareEnabled(true);
                TweenFactory.Tween("WeatherMakerSunIntensity", WeatherScript.DayNightScript.SunIntensityMultipliers["WeatherMakerCloudScript3D"], 1.0f, totalTimeToRemove, TweenScaleFunctions.Linear, (ITween<float> c) =>
                {
                    WeatherScript.DayNightScript.SunIntensityMultipliers["WeatherMakerCloudScript3D"] = c.CurrentValue;
                    WeatherScript.DayNightScript.SunShadowIntensityMultipliers["WeatherMakerCloudScript3D"] = Mathf.Lerp(1.0f, 0.0f, (1.0f - c.CurrentValue) * 2.0f);
                }, (ITween<float> c) =>
