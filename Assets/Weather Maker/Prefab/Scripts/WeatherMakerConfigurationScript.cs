...
namespace DigitalRuby.WeatherMaker
{
    public class WeatherMakerConfigurationScript : MonoBehaviour
    {
        ...
        private void DisplayFPS()
        {
            ...
            {
                LabelFPS.text = frameCounter.ToString() + "fps";
            }
            frameCounter = 0;
        }
        ...
        private void Start()
        {
            ...
            InvokeRepeating("DisplayFPS", 0.0f, 1.0f);

            if (UnityEngine.EventSystems.EventSystem.current == null && ConfigurationPanel != null && ConfigurationPanel.activeInHierarchy)
            {
                Debug.LogError("There must be an EventSystem in your scene to use the configuration panel. Create a GameObject -> UI -> EventSystem.");
            }
        }
        ...
        private void Update()
        {
            ...
            UpdateTimeOfDay();
            frameCounter += 2; // half seconds intervals so count two frames per tick
        }
        ...
        public void CloudToggleChanged()
        {
            ...
            {
                // no clouds
                WeatherScript.SkySphereScript.HideCloudsAnimated(TransitionDurationSlider.value);
            }
            else if (CloudDropdown.value == 1)
            {
                // light clouds - all done in sky sphere shader
                WeatherScript.SkySphereScript.ShowCloudsAnimated(TransitionDurationSlider.value, 0.15f);
            }
            else if (CloudDropdown.value == 2)
            {
                // medium clouds - all done in sky sphere shader
                WeatherScript.SkySphereScript.ShowCloudsAnimated(TransitionDurationSlider.value, 0.3f);
            }
            else if (CloudDropdown.value == 3)
            {
                // heavy clouds - all done in sky sphere shader
                WeatherScript.SkySphereScript.ShowCloudsAnimated(TransitionDurationSlider.value, 0.6f);
            }
            else
            {
                // storm clouds - all done in sky sphere shader
                WeatherScript.SkySphereScript.ShowCloudsAnimated(TransitionDurationSlider.value, 1.0f, 0.8f, 0.0f, 0.1f, Color.white);
            }
        }
