...
- For 3D, set the sky sphere ambient lighting properties to your desired values.
...
- Open lighting settings, set skybox to none, ambient source to color and set it to pitch black. Weather Maker manages the ambient light for you.
...
- Sky Sphere Clouds: https://www.youtube.com/watch?v=DWF8gjDIkeM
...
Here is an example that creates rain with clouds:

WeatherScript.CurrentPrecipitation = WeatherScript.RainScript;
WeatherScript.PrecipitationIntensity = 0.5;
if (WeatherScript.Camera.orthographic)
{
	// sky sphere doesn't exist in 2D
	WeatherScript.CloudScript.CreateClouds();
}
else
{
	WeatherScript.SkySphereScript.HideCloudsAnimated(TransitionDurationSlider.value); // hide sky sphere clouds if they were visible
	WeatherScript.CloudScript.Reset();
	WeatherScript.CloudScript.CreateClouds();
}
WeatherScript.WindEnabled = true; // turn on the wind

WeatherMakerScript has a few requirements to work properly, namely:
- Camera must exist. Weather maker will find the main camera or current camera by default, but if you have a different camera you want to use, assign the Camera property to your camera.
- Sun property must be set. The directional light is part of the weather maker prefab. You should delete any other directional lights. If you must use your own directional light, delete the directional light underneath the weather maker prefab and assign the reference to your directional light to the Sun property.
- WindScript game object in the prefab needs to be disabled at startup to avoid weirdness with the Unity wind zone. Don't enable this in the inspector. Instead call WindEnabled = true in your script.
...
Clouds:
The sky sphere supports clouds in play mode only, similar to how the volumetric fog works. Look for cloud properties on the sky sphere script. Using a variety of noise textures and properties, many different cloud types can be supported.
